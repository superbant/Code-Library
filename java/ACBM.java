...
public class ACBM {
	...
	public static void main(String[] args) {
		...
		String text = "testabtest";
		String[] patternArray = { "test", "ab" };
		Map<String, List<Integer>> map = acbm(text, patternArray);
	 ...
	 *            结果集合
	 */
	private static void setNodeToMap(BMTrieNode currentNode, String[] patternArray,
	...
	private static void buildMatchStr(BMTrieNode root, String[] patternArray) {
		if (root != null) {
			...
			for (int i = 0; i < patternArray.length; i++) {
				if (root.getMatchStr() != null && root.getMatchStr().startsWith(patternArray[i])) {
...
class BMTrieNode {
	 ...
	 * @param compareAt
	 *            待匹配的位置
	 * @param badCharacterMap
	 *            错误字典表
	 * @return 需要移动的位置
	 */
	public Integer matchCurrentNode(char[] textArray, int alingnAt, int compareAt,
			Map<Character, Integer> badCharacterMap) {
		...
		if (preNode != null) {
			...
			if (alingnAt + 1 >= textArray.length) {
				return 1;
			}
	...
	public String toString() {
		return "matchStr:" + matchStr + ", " + "status:" + status + ", pattern"
